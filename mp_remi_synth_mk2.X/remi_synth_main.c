 ...
 * `````        build variants, except for "remi_synth_GUI_lite.#".  
 *              Hardware dependencies are confined to files...
...
PRIVATE  void   PowerManagementTask(void);  
PRIVATE  void   BatteryChargeProcess(void); 
#endif
...
static  uint16  m_BatteryVoltageAve_mV;  // Battery voltage, average rdg (Lite variant)
...
#ifdef SYNTH_MK2_MX340_LITE  // use I2C OLED display
    {
        ...
        SH1106_Wake();
        Disp_ClearScreen();
        BATT_CHARGE_DISABLE();  // Assume battery OK at startup
    }
...
#ifdef SYNTH_MK2_MX340_LITE
/**
 * Periodic task called at 50 millisecond intervals.
 * Checks power source;  if battery, disable CLI UART; else enable CLI UART.
 * Measures battery voltage and computes running average (filtered) reading.
 * Updates battery status LED.  If battery is low (under 2.3V) the LED is pulsed
 * at 1/2Hz (50ms duty).  If battery is fully charged (over 2.6V) the LED is
 * turned on continuously.
 */
PRIVATE void  PowerManagementTask()
{
    static  uint8  LED_timer;  // unit = 50ms
    static  bool   isBatteryPowered;   // last state
    static  bool   isFullCharge; 
    static  int    battVoltageFilter;  // fixed-point (24:8 bits)
    
    int  batteryReading_mV = ((int) AnalogResult(6) * 3300) / 1023;
    
    // One-time initialization at power-on/reset...  convert to fixed-pt
    if (battVoltageFilter == 0) battVoltageFilter = batteryReading_mV << 8;
    
    // Apply first-order IIR filter (running average) algorithm...
    battVoltageFilter -= battVoltageFilter >> 4;  // filter K = 1/16
    battVoltageFilter += batteryReading_mV << 4;
    m_BatteryVoltageAve_mV = battVoltageFilter >> 8;  // get integer part
    
    if (!isBatteryPowered && BATTERY_POWER_ON)  // switched to battery power
    {
        isBatteryPowered = TRUE;
        UART2_DISABLE();     // Disable CLI UART -- reduces power consumption
        U2TX_PIN_SET_LOW();  // Prevent current leakage into USB/UART RXI pin
    }
    if (isBatteryPowered && !BATTERY_POWER_ON)  // switched to USB power
    {
        isBatteryPowered = FALSE;
        UART2_ENABLE();  // Enable UART2 for CLI operation
    }
    
    if (BATTERY_POWER_ON && m_BatteryVoltageAve_mV >= 2650)  // full charge
    {
        BATT_STATUS_LED_ON();  
        isFullCharge = TRUE;
    }
    if (m_BatteryVoltageAve_mV < 2600) isFullCharge = FALSE;  // use hysteresis
    if (m_BatteryVoltageAve_mV < 2300)  // low battery
    {
        if (LED_timer == 0)  BATT_STATUS_LED_ON();  // LED on for 50ms
        else  BATT_STATUS_LED_OFF();
        if (++LED_timer >= 40)  LED_timer = 0;  // 2-sec period ended
    }
    else if (!isFullCharge)  BATT_STATUS_LED_OFF();  // normal state
}
 ...
 * <!> Deprecated function
 * ```````````````````````
 * The Lite synth hardware design has been revised such that the MCU no longer has
 * control over the battery charging operation.  The battery is always charged 
 * while USB power is connected.  Charge current is very low (50mA max.) so that
 * the battery will not be damaged, even when fully charged.
 * 
 * Periodic task called at 5 second intervals.
 * While USB power is available (VBUS ON), the battery charging circuit is enabled
 * or disabled according to the battery voltage measured.
 * The charging circuit is disabled by hardware while battery power is switched on.
 * 
 * A charge cycle consists of 5 minutes charging followed by one minute rest to allow
 * the battery voltage to settle. At the end of each cycle, the battery voltage is
 * measured and, if fully charged (at 2700mV), the charging function is suspended.
 **
PRIVATE void  BatteryChargeProcess(void)
{
    static unsigned  chargeCycleTimer;  // unit = 5 seconds
    
    chargeCycleTimer++;
    
    if (!BATTERY_POWER_ON && g_Config.BatteryChargeFlag)  // Charging state
    {
        if (chargeCycleTimer <= 59) BATT_CHARGE_ENABLE();   // First 5 minutes
        if (chargeCycleTimer == 60) BATT_CHARGE_DISABLE();  // At 5 minute mark
        if (chargeCycleTimer >= 72)  // After a minute without charging...
        {
            chargeCycleTimer = 0;  // Start new charge cycle
            // Check if reached full charge, or battery fault
            if (m_BatteryVoltageAve_mV < 1900 || m_BatteryVoltageAve_mV >= 2700)  
            {
                BATT_CHARGE_DISABLE();
                g_Config.BatteryChargeFlag = 0;  // Suspend charging function
                StoreConfigData();
            }
        }
    }
    else if (!g_Config.BatteryChargeFlag)  // Not charging -- Check for low battery
    {
        if (m_BatteryVoltageAve_mV >= 1900 && m_BatteryVoltageAve_mV < 2100)
        {
            g_Config.BatteryChargeFlag = 1;  // Activate charging function
            StoreConfigData();
        }
    }
}
...
/*
 * Function returns battery voltage reading average in millivolts.
 */
int  GetBatteryVoltage(void)
{
    return (int) m_BatteryVoltageAve_mV;
}

#endif  // SYNTH_MK2_MX340_LITE
